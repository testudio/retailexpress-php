...
class RetailExpressClient {
   ...
   *
   * @param string $apiKey
   *   Your permanent Retail Express API key (for the x-api-key header)
   * @param \GuzzleHttp\ClientInterface|null $client
   *   An optional Guzzle client instance for testing.
   * @param string $base
   *   Base URL for the API (default: Retail Express API URL)
   * @param string $version
   *   API version for all endpoints (default: v2.1)
   */
   ...
   *   Optional query string parameters for GET
   *
  ...
  public function getProductPrices(int $page_number = 1, int $page_size = self::MAX_PAGE_SIZE): array {
    $params = ['page_number' => $page_number, 'page_size' => $page_size];
    return $this->request('GET', 'productprices', [], $params);
  }
